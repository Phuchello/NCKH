...
* **Snapshot-Pinned Provenance Invariant**: All machine-extracted intelligence entities (`claims`, `evidence_items`, `idea_provenance`, `document_chunks`) enforce `snapshot_id NOT NULL`. The schema uses `ON DELETE RESTRICT` on `claims`, `evidence_items`, and `idea_provenance` to prevent silently orphaning extracted knowledge when a snapshot is deleted. A snapshot with dependent intelligence records cannot be dropped without explicitly migrating or removing those records first.
